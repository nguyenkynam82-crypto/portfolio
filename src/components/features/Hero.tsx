      ...
      <div className="relative z-10 w-full flex flex-col mt-2 md:mt-4">
        ...
        >
          <h1 data-cursor-dark className="font-heading font-black uppercase tracking-tight">
            ...
            <span className={`block ${isEn ? 'text-3xl sm:text-5xl md:text-6xl lg:text-[4.5rem] xl:text-[5.5rem]' : 'text-6xl sm:text-7xl md:text-8xl lg:text-[9rem] xl:text-[11rem]'} text-white leading-[1.05]`}>{t('hero.title1')}</span>
            <span className={`text-mask-gradient inline-block ${isEn ? 'text-5xl sm:text-6xl md:text-7xl lg:text-[7.5rem] xl:text-[9rem]' : 'text-7xl sm:text-8xl md:text-9xl lg:text-[10rem] xl:text-[12rem]'} leading-[1.1] py-2`}>{t('hero.title2')}</span>
          </h1>
        </motion.div>
      </div>
